...
const App: React.FC = () => {
  // Sort games alphabetically by Korean title using useMemo to optimize performance
                ...
                {[1, 2, 3].map((num) => {
                  if (num === 2) {
                    return (
                      <span
                        key={num}
                        className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-black flex items-center justify-center text-white font-bold text-xs md:text-sm border border-zinc-700 shadow-[0_0_10px_rgba(255,255,255,0.1)] cursor-default"
                      >
                        {num}
                      </span>
                    );
